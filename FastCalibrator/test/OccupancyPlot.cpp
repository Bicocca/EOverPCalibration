...
for(int iEta =0; iEta<h_OccupancyEB->GetNbinsY() ;iEta++){
 ...
 if(iEta<85 && sumHit!=0 && nHit!=0 ){ OccupancyEB_vs_Eta->SetPoint(nPoint,-(85-iEta)*xtalWidth,sumHit/(nHit*luminosity));
...
// Graph for EB
nPoint=0; nPointAll =0; nPointAll2 =0;
...
sumHitEEM.assign(40,0.); nHitEEM.assign(40,0);
std::vector<double> sumHitEEP ;
std::vector<int> nHitEEP ;
sumHitEEP.assign(40,0.); nHitEEP.assign(40,0);
...
for(int k =0; k<2 ; k++){
 for(int ix =0; ix<h_OccupancyEE[k]->GetNbinsX() ;ix++){
   ...
   if(h_OccupancyEE[k]->GetBinContent(ix+1,iy+1)==0) continue;
    ...
    if(k==0){ sumHitEEM.at(ring)=sumHitEEM.at(ring)+occ; nHitEEM.at(ring)=nHitEEM.at(ring)+1;}
    if(k==1){ sumHitEEP.at(ring)=sumHitEEP.at(ring)+occ; nHitEEP.at(ring)=nHitEEP.at(ring)+1;}
   }
 }
}
