...
int main (int argc, char** argv){
 ...
 try{ PhiStartDeadXtal =  gConfigParser -> readIntOption("Input::PhiStartDeadXtal"); }
 catch(char const* exceptionString ){ PhiStartDeadXtal=-999.; }

 int PhiStepDeadXtal ;
 try{ PhiStepDeadXtal  = gConfigParser -> readIntOption("Input::PhiStepDeadXtal");}
 catch(char const* exceptionString ){ PhiStepDeadXtal=-999.; }

 int PhiNShiftDeadXtal ;
 try{ PhiNShiftDeadXtal = gConfigParser -> readIntOption("Input::PhiNShiftDeadXtal");}
 catch(char const* exceptionString ){ PhiNShiftDeadXtal = -999.;}
 ...
 try{ IxEEPBinCenterDeadXtal = gConfigParser -> readIntListOption("Input::IxEEPBinCenterDeadXtal"); }
 catch(char const* exceptionString ){ IxEEPBinCenterDeadXtal.push_back(-999.); }

 std::vector<int> IxEEMBinCenterDeadXtal ;
 try{ IxEEMBinCenterDeadXtal = gConfigParser -> readIntListOption("Input::IxEEMBinCenterDeadXtal"); }
 catch(char const* exceptionString ){ IxEEMBinCenterDeadXtal.push_back(-999.); }

 std::vector<int> IyEEPBinCenterDeadXtal ;
 try{ IyEEPBinCenterDeadXtal = gConfigParser -> readIntListOption("Input::IyEEPBinCenterDeadXtal"); }
 catch(char const* exceptionString ){ IyEEPBinCenterDeadXtal.push_back(-999.); }

 std::vector<int> IyEEMBinCenterDeadXtal ;
 try{ IyEEMBinCenterDeadXtal = gConfigParser -> readIntListOption("Input::IyEEMBinCenterDeadXtal"); }
 catch(char const* exceptionString ){ IyEEMBinCenterDeadXtal.push_back(-999.); }

 int iXNShiftDeadXtal ;
 try{ iXNShiftDeadXtal = gConfigParser -> readIntOption("Input::iXNShiftDeadXtal");}
 catch(char const* exceptionString ){ iXNShiftDeadXtal = -999.;}

 int iYNShiftDeadXtal ;
 try{ iYNShiftDeadXtal = gConfigParser -> readIntOption("Input::iYNShiftDeadXtal");}
 catch(char const* exceptionString ){ iYNShiftDeadXtal = -999.;}

 std::string OutputDeadXtalPath ;
 try{ OutputDeadXtalPath = gConfigParser -> readStringOption("Input::OutputDeadXtalPath");
      system(("mkdir -p "+OutputDeadXtalPath).c_str());}
 catch(char const* exceptionString ){ OutputDeadXtalPath = "cfg/InputDeadXtal/" ;
                                      system(("mkdir -p "+OutputDeadXtalPath).c_str());
 }

 std::string OutputDeadXtalFile ;
 try{ OutputDeadXtalFile = gConfigParser -> readStringOption("Input::OutputDeadXtalFile");}
 catch(char const* exceptionString ){ OutputDeadXtalFile = "DeadXtalFile"; }


 std::string inputCfgTemplate ;
 try{ inputCfgTemplate = gConfigParser -> readStringOption("Input::inputCfgTemplate");}
 catch(char const* exceptionString ){ inputCfgTemplate = "cfg/FastCalibrator_Template.cfg"; }
 ...
 try{ OutputCfgPath = gConfigParser -> readStringOption("Output::OutputCfgPath");
      system(("mkdir -p "+OutputCfgPath).c_str()); }
 catch(char const* exceptionString ){ OutputCfgPath = "cfg/JOB/";
                                     system(("mkdir -p "+OutputCfgPath).c_str());
 }
 ...
 if(IxEEPBinCenterDeadXtal.size() != IyEEPBinCenterDeadXtal.size() ) { std::cout<<" Error in EEP Dead Xtal List --> exit "<<std::endl; return -1 ;}
 if(IxEEMBinCenterDeadXtal.size() != IyEEMBinCenterDeadXtal.size() ) { std::cout<<" Error in EEM Dead Xtal List --> exit "<<std::endl; return -1 ;}
